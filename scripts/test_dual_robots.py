...
class DualRobotController:
    ...
    def connect(self):
        ...
        self.robot1 = RemoteRobot(
            robot_id="robot_001", robot_type="so100_follower", host=self.host, port=self.port
        )
        self.robot1.connect()
        ...
        self.robot2 = RemoteRobot(
            robot_id="robot_002", robot_type="so100_follower", host=self.host, port=self.port
        )
        self.robot2.connect()
    ...
    def opposite_movement_test(self):
        ...
        tests = [("shoulder_lift", 15, -15), ("elbow_flex", -20, 20), ("wrist_flex", 25, -25)]
        ...
        for motor_name, delta1, delta2 in tests:
            ...
            success1, success2 = self.send_actions(action1, action2)
            print(
                f"  Commands sent: Robot1 {'✓' if success1 else '✗'}, Robot2 {'✓' if success2 else '✗'}"
            )
