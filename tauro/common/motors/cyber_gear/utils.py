...
def float_to_uint(value: float, range_min: float, range_max: float, bits: int) -> int:
    """
    Converts a floating-point number into an unsigned integer with a specified bit depth
    while respecting a given range.
...
def uint_to_float(value: int, value_min: float, value_max: float) -> float:
    """
    This is the opposite of float_to_uint. It takes an unsigned integer received from
    the CAN bus and converts it back into a floating point number
    """
    int_max = 0xFFFF
...
def extract_type(data: bytearray, to_type: DataType) -> int:
    """
